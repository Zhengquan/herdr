...
クライアントが接続していないとき、サーバーはレイアウトと新規作成ペインに 120×40 の仮想ターミナルを使います。ヘッドレスなオーケストレーション向けにこのフォールバックを変更するには次のように設定します:
...
接続中のクライアントは共有ランタイムサイズの権威であり続けます。切断後は、既存ペインの PTY が最後に接続していたときのサイズを保持し、新しいヘッドレスレイアウトは設定されたフォールバックを使います。
