...
| --- | --- | --- |
| ライフサイクル権威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode、CodeBuddy、WorkBuddy（ブリッジ watcher） | インストールされ、そのペインについて能動的に報告している間は、フックまたはプラグインのイベントが `idle`、`working`、`blocked` を決定します。同じライフサイクル権威に対して、Herdr はスクリーンマニフェストのフォールバックを併用しません。 |
...
CodeBuddy フックをインストールします:

```bash
herdr integration install codebuddy
```

CodeBuddy フックは、ほかのインテグレーションと同じローカル socket API を通じてセッション識別とライフサイクル状態を報告します。インストールされている間、このフックが `idle`、`working`、`blocked` のライフサイクル権威になります。その権威に対して、Herdr はスクリーンマニフェストのフォールバックを併用しません。

Herdr はデフォルトで `~/.codebuddy` を使い、`CODEBUDDY_HOME` が設定されていればそちらを使います。CodeBuddy の設定ディレクトリはあらかじめ存在している必要があります。インストールは `hooks/herdr-agent-state.sh`（Windows では `hooks/herdr-agent-state.ps1`）を書き込み、`settings.json` のフックを更新し、`SessionStart`、`UserPromptSubmit`、`PreToolUse`、`Stop`、`Notification` のエントリを登録します。アンインストールは `settings.json` から Herdr のエントリを削除し、フックスクリプトを削除します。

CodeBuddy がセッション情報を含むイベントを発行した後、Herdr は報告されたセッション id を使って `codebuddy --resume <id>` でペインを resume できます。

## WorkBuddy

WorkBuddy は単体の macOS デスクトップアプリです。Herdr のペイン内では決して動かないため、プロセス検出、スクリーンマニフェスト、フックベースのインテグレーションでは観測できません。そこで WorkBuddy インテグレーションは、代わりにブリッジ watcher をインストールします:

```bash
herdr integration install workbuddy
```

インストールは `~/.workbuddy/herdr/herdr-workbuddy-watch.sh` を書き込み、Herdr サーバーが動いている場合は "WorkBuddy" ワークスペースを自動的に作成してそのルートペインで watcher を起動します。watcher は WorkBuddy のローカル状態を読み取り専用でポーリングし（セッション一覧はセッションデータベース、ターンのライフサイクルは会話トランスクリプトから取得）、集約した状態を socket API 経由で報告します（いずれかのセッションがユーザーの回答待ちなら `blocked`、ほかのセッションでターンが進行中なら `working`、すべてのターンが終わっていれば `idle`、ステータスメッセージは現在のタスクタイトル）。`HERDR_WORKBUDDY_NO_AUTOSPAWN=1` を設定するとスクリプトだけをインストールし、その後どの Herdr ペインでも `sh ~/.workbuddy/herdr/herdr-workbuddy-watch.sh` を実行してブリッジを手動で起動できます。

1 つのブリッジペインは個々のセッションではなく WorkBuddy アプリ全体を表します。アンインストールは watcher スクリプトを削除し、動作中のブリッジは次のポーリングでそれに気づいて終了します。
