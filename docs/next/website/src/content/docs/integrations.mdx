---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Cursor Agent CLI, MastraCode, Antigravity CLI, Grok CLI, and CodeBuddy.
---
...
herdr integration install workbuddy
```
...
herdr integration uninstall workbuddy
```
...
| --- | --- | --- |
| Lifecycle authority | Pi, OMP, Kimi Code CLI, OpenCode, Kilo Code CLI, MastraCode, CodeBuddy, WorkBuddy (bridge watcher) | When installed and actively reporting for the pane, hook or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
...
WorkBuddy is a standalone macOS desktop app: it never runs inside a Herdr pane, so process detection, screen manifests, and hook-based integrations cannot observe it. The WorkBuddy integration instead installs a bridge watcher:

```bash
herdr integration install workbuddy
```

Install writes `~/.workbuddy/herdr/herdr-workbuddy-watch.sh` and, when a Herdr server is running, automatically creates a "WorkBuddy" workspace and starts the watcher in its root pane. The watcher polls WorkBuddy's local state read-only — its session database for the session list and its conversation transcripts for turn lifecycle — and reports the aggregated state (`blocked` while any session is waiting for a user answer, `working` while any other session has a turn in flight, and `idle` once every turn has ended, with the current task title as the status message) through the socket API. Set `HERDR_WORKBUDDY_NO_AUTOSPAWN=1` to install only the script, then start the bridge manually by running `sh ~/.workbuddy/herdr/herdr-workbuddy-watch.sh` inside any Herdr pane.

One bridge pane represents the whole WorkBuddy app, not individual sessions. Uninstall removes the watcher script; a running bridge notices and exits on its next poll.
