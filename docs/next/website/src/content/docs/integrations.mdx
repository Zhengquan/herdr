...
Install writes `~/.workbuddy/herdr/herdr-workbuddy-watch.sh` and, when a Herdr server is running, automatically creates a "WorkBuddy" workspace and starts the watcher in its root pane. The watcher polls WorkBuddy's local state read-only — its session database for the session list and its conversation transcripts for turn lifecycle — and reports the aggregated state (`working` while any session has a turn in flight, `idle` once every turn has ended, with the current task title as the status message) through the socket API. Set `HERDR_WORKBUDDY_NO_AUTOSPAWN=1` to install only the script, then start the bridge manually by running `sh ~/.workbuddy/herdr/herdr-workbuddy-watch.sh` inside any Herdr pane.
