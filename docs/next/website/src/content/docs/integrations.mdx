...
Install writes `~/.workbuddy/herdr/herdr-workbuddy-watch.sh` and, when a Herdr server is running, automatically creates a "WorkBuddy" workspace and starts the watcher in its root pane. The watcher polls WorkBuddy's local session database read-only and reports the aggregated state (`working` when any session is active, `blocked` when the most recent task failed, `idle` otherwise, with the current task title as the status message) through the socket API. Set `HERDR_WORKBUDDY_NO_AUTOSPAWN=1` to install only the script, then start the bridge manually by running `sh ~/.workbuddy/herdr/herdr-workbuddy-watch.sh` inside any Herdr pane.
