...
# local session database read-only, and reports the aggregated state for this
# pane over the Herdr socket API.
...
def current_state():
    """Return (state, message) aggregated from WorkBuddy's session database."""
    if not os.path.isfile(db_path):
        return ("idle", "WorkBuddy database not found")
    try:
        db = sqlite3.connect(f"file:{db_path}?mode=ro", uri=True)
        rows = db.execute(
            "SELECT status, title FROM sessions "
            "WHERE deleted_at IS NULL AND status != 'archived' "
            "ORDER BY last_activity_at DESC LIMIT 20"
        ).fetchall()
        db.close()
    except Exception:
        return ("idle", "WorkBuddy database unreadable")

    working = [r for r in rows if str(r[0]).lower() == "working"]
    if working:
        title = working[0][1] or "task"
        return ("working", str(title)[:120])

    failed = [r for r in rows if str(r[0]).lower() in ("error", "failed")]
    if failed:
        title = failed[0][1] or "task"
        return ("blocked", f"task failed: {str(title)[:110]}")
